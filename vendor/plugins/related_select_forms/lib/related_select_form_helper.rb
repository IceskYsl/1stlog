...
#      logger.info("===result:#{result}==record:#{record}")
      reference_value = record.send(reference_method).to_s
#      logger.info("\r=====reference_value:#{reference_value}")
      record_value    = record.send(value_method).to_s
#      logger.info("\r==========record_value:#{record_value}")
#      logger.info("\r==========text_method:#{record.send(text_method)}")
